<div class="content" id="content">
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="header">
						...
						<a href="<?php echo base_url('townhouse/addtownhouse'); ?>" class="btn btn-primary">
	...
	<table class="table table-hover table-striped" id="table_id" >
     ...
     <tbody>
	   ...
	   <?php 
	   foreach($townhouse as $row ) : 
	  
	   ?>
	   <tr id="<?php echo $row['id_twnhouse']; ?>">	
	   		<?php if($user['role_id'] == 1) { ?>
			<?php } ?>
            <td><input type="checkbox" class="emp_checkbox" data-emp-id="[<?php echo $row['id_twnhouse']; ?>]"></td>
            ...
            <a href="<?php echo base_url('townhouse/updatetownhouse/'.$row['id_twnhouse']);?>"><i class="fa fa-edit"></i></a>
            </button>
            <a href="<?php echo base_url('townhouse/deletetownhouse/'.$row['id_twnhouse']);?> "><button type="button" rel="tooltip" title="Remove" class="btn btn-danger btn-simple btn-xs">
    ...
    <button type="button" id="updateRecorToHO"  class="btn btn-primary">Update HO</button>
    <button type="button" id="updateRecorToPHO"  class="btn btn-primary">Update Pending HO</button>
    <button type="button" id="updateRecorToSTS"  class="btn btn-primary">Update STS</button>
    <p id="demo"></p>
    </div>
...
$(document).ready(function(){
    ...
    $("#updateRecorToPHO").show(); 
    $("#updateRecorToSTS").show();
    ...
    $('#updateRecorToHO').on('click', function(e){
    ...
    if(towerOne.length <=0) { alert("Please select records update."); 
    } else { WRN_PROFILE_DELETE = "Are you sure you want to update "+(towerOne.length>1?"these":"this")+" row?";
    ...
    var vStatus = '1';
                $.ajax({
                method: "POST",
                url: "<?php echo base_url();?>townhouse/multiupdatehotownhouse",
    ...
    $('#updateRecorToPHO').on('click', function(e){
    ...
    $('#updateRecorToSTS').on('click', function(e){
    ...
    $("#delete_records").on('click', function(e){
        ...
        var selected_values = towerOne.join();
                    $.ajax({
                    method: "POST",
                    url: "<?php echo base_url();?>townhouse/deletemultipletownhouse",
                    data: 'emp_id='+selected_values,
                    success: function(response) {
                    // remove deleted employee rows
                    // var x = document.getElementById("demo");
